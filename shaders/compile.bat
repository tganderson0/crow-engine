C:\VulkanSDK\1.3.250.0\Bin\glslc.exe tri_mesh_ssbo.vert -o tri_mesh_ssbo.vert.spv
C:\VulkanSDK\1.3.250.0\Bin\glslc.exe default_lit.frag -o default_lit.frag.spv

C:\VulkanSDK\1.3.250.0\Bin\glslc.exe tri_mesh.vert -o tri_mesh.vert.spv
C:\VulkanSDK\1.3.250.0\Bin\glslc.exe textured_lit.frag -o textured_lit.frag.spv

C:\VulkanSDK\1.3.250.0\Bin\glslc.exe pbr_lit.frag -o pbr_lit.frag.spv
C:\VulkanSDK\1.3.250.0\Bin\glslc.exe tri_mesh_normal_ssbo.vert -o tri_mesh_normal_ssbo.vert.spv

pause